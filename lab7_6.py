...
"""
import os
# first=input("파일 주소를 입력하세요 : ")      # "C:\\Temp\\201732028"
first="C:\\Temp\\201732028"
os.chdir(first)
while (1):
    position = os.getcwd()
    dirlist = os.listdir(position)
    if len(dirlist) != 0:
        for file in dirlist:
            u = os.path.join(position, file)
            if os.path.isdir(u):
                if len(os.listdir(u))==0:
                    ...
                    os.chdir(os.path.join(position, file))
